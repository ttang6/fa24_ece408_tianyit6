...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    int OutputSize = Batch * Map_out * (Height - K + 1) * (Width - K + 1) * sizeof(float);
    int kernelSize = Map_out * Channel * K * K * sizeof(float);

    cudaMalloc((void **)device_input_ptr, InputSize);
    cudaMalloc((void **)device_output_ptr, OutputSize);
    cudaMalloc((void **)device_mask_ptr, kernelSize);

    cudaMemcpy(*device_input_ptr, host_input, InputSize, cudaMemcpyHostToDevice);
    cudaMemcpy(*device_mask_ptr, host_mask, kernelSize, cudaMemcpyHostToDevice);
}
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    int h = ceil((Height - K + 1) / (1.0*TILE_WIDTH));
    int w = ceil((Width - K + 1) / (1.0*TILE_WIDTH));
    
    dim3 DimBlock(TILE_WIDTH, TILE_WIDTH, 1);
    dim3 DimGrid(Batch, Map_out, h*w);
