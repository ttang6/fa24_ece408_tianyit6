                                     ...
                                     int numCRows, int numCColumns) 
{
    ...
    float registerC = 0.0f;
    
    float regA[TILE_WIDTH];
    float regB[TILE_WIDTH];
    ...
    if (row < numCRows && col < numCColumns)
        C[row * numCColumns + col] = registerC;
}
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    matrixMultiplySharedOptimized<<<DimGrid, DimBlock>>>(device_mask, unrolled_matrix, matmul_output, Map_out, Height_unrolled, Height_unrolled, Width_unrolled, Map_out, Width_unrolled);
