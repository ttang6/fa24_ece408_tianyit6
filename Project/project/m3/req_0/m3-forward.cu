...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    float *pin_input, *pin_output;
    cudaHostAlloc(&pin_input, inputSize * sizeof(float), cudaHostAllocDefault);
    cudaHostAlloc(&pin_output, outputSize * sizeof(float), cudaHostAllocDefault);
    memcpy(pin_input, host_input, inputSize * sizeof(float));
    ...
    cudaMemcpyAsync(*device_input_ptr, pin_input, inputSize * sizeof(float), cudaMemcpyHostToDevice, stream1);
    cudaMemcpyAsync(*device_mask_ptr, host_mask, maskSize * sizeof(float), cudaMemcpyHostToDevice, stream2);
    ...
    matrix_unrolling_kernel<<<DimGrid, DimBlock, 0, stream1>>>(*device_input_ptr, unrolled_matrix, Batch, Channel, Height, Width, K);
    ...
    matrixMultiplyShared<<<DimGrid, DimBlock, 0, stream1>>>
        (*device_mask_ptr, unrolled_matrix, matmul_output, Map_out, Height_unrolled, Height_unrolled, Width_unrolled, Map_out, Width_unrolled);

    const int out_image_size = Height_out * Width_out;
    dim3 permute_grid_dim((out_image_size - 1) / BLOCK_SIZE + 1, Batch, 1);
    matrix_permute_kernel<<<permute_grid_dim, BLOCK_SIZE, 0, stream1>>>
        (matmul_output, *device_output_ptr, Map_out, Batch, out_image_size);

    cudaMemcpyAsync(pin_output, *device_output_ptr, outputSize * sizeof(float), cudaMemcpyDeviceToHost, stream1);

    cudaStreamSynchronize(stream1);
    cudaStreamSynchronize(stream2);

    memcpy((void*)host_output, pin_output, outputSize * sizeof(float));

    cudaStreamDestroy(stream1);
    cudaStreamDestroy(stream2);
    cudaFreeHost(pin_input);
    cudaFreeHost(pin_output);
