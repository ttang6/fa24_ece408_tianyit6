    ...
    if(t < Channel * W_unroll)
    {
        ...
        for (int p = 0; p < K; p++) {
            for (int q = 0; q < K; q += 4) 
            {
                ...
                if (q + 0 < K)
                    output[(h_unroll + 0) * W_unroll + w_unroll] = in_4d(b, c, h_out + p, w_out + q + 0);
                if (q + 1 < K)
                    output[(h_unroll + 1) * W_unroll + w_unroll] = in_4d(b, c, h_out + p, w_out + q + 1);
                if (q + 2 < K)
                    output[(h_unroll + 2) * W_unroll + w_unroll] = in_4d(b, c, h_out + p, w_out + q + 2);
                if (q + 3 < K)
                    output[(h_unroll + 3) * W_unroll + w_unroll] = in_4d(b, c, h_out + p, w_out + q + 3);
            }
        }
    }
